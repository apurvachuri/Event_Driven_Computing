...
public class FsaEditor {
    ...
    private final JFrame window = new JFrame("Fsa Editor GUI");
        ...
        public void paintComponent(Graphics g) {
            ...
            if (fsaPanel.getFsa().isRecognised()) {
               // g.drawString("Sequence Recognised", 10, 30);
                ...
                eventInput.setBackground(Color.GREEN);
            } else {
               // g.drawString("Sequence Not Recognised", 10, 30);
                System.out.println("Sequence not recognised");
                eventInput.setBackground(Color.WHITE);
            }
        }
    };
    ...
    public FsaEditor() {
        ...
        //control panel
        resetButton.setBounds(35, 480, 70, 40);
        resetButton.addActionListener(actionListener);
        window.add(resetButton);

        stepButton.setBounds(130, 480, 70, 40);
        stepButton.addActionListener(actionListener);
        window.add(stepButton);

        eventInputLabel.setBounds(225, 480, 120, 40);
        window.add(eventInputLabel);

        eventInput.setBounds(325, 480, 250, 40);
    ...
    public void openFile(String fileName) {
        ...
        try {
            ...
            r.close();
            fsaLoaded = true;
            window.setTitle("Fsa Editor App - " + fileName);
    ...
    public void openFile() {
        ...
        if (res == 0) {
            try {
                ...
                r.close();
                fsaLoaded = true;
                window.setTitle("Fsa Editor App - " + fc.getSelectedFile().getName());
            } catch (FileNotFoundException d) {
                window.remove(fsaPanel);
                fsaPanel = old;
                window.add(fsaPanel);
    ...
    public void saveFile() {
        ...
        if (res == 0) {
            try {
                ...
                window.setTitle("Fsa Editor App - " + s);
                FsaReaderWriter fsaRW = new FsaReaderWriter();
